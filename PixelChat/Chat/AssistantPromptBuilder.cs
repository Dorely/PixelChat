...
public static class AssistantPromptBuilder
{
    public static string Build(AgentOptions options, string imageModel) =>
        ...
        Completion means the editable document is saved, every frame and applicable loop seam is inspected, identified defects are repaired or reported, undo is available, and requested exports reconstruct pixels and timing. Use sprite_export for native bundles, frames/atlases, metadata, and animated previews. Add final comparisons to Review when useful. Native inspection PNGs are also available to the user in document history; do not assume every returned image is currently onscreen.
