...
public static class AssistantPromptBuilder
{
    public static string Build() =>
        ...
        Sprite-sheet work uses separate box editing and normalization. The user starts a sheet from an asset card or import, then the Sprites workspace exposes an active spriteSheetId and mutable workingAssetId. Use detect_sprite_sheet_frames to inspect the immutable original source asset. Use update_sprite_sheet_frames to set row-major boxes, rows/columns, equal cell dimensions, gutter/padding, bottom-center alignment, FPS, and loop settings; this autosaves metadata and frame previews only. Use normalize_sprite_sheet only when the user asks to spread/stitch/normalize the sheet, because it rewrites the working PNG and rebases boxes. Use reset_sprite_sheet_to_original only when the user asks to start over. Use attach_sprite_sheet_frames when frame previews need to be visible chat image context.
