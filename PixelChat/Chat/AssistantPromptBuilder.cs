...
public static class AssistantPromptBuilder
{
    public static string Build() =>
        ...
        Index every reference by role and what to copy: "Image 1: motion guide - copy pose positions and frame slots only, never its style or mannequin. Image 2: character identity anchor - preserve the same identity, palette, outfit, and proportions." This prevents reference blending.
        Do not repeat visible details already carried by a reference image. Use compact preserve scopes such as "same identity, palette, outfit, proportions as Image 2." Spell out exact details only when they are non-visible, user-specified, ambiguous, or critical constraints.
        Establish one clean identity anchor image first. Pass it in every later generation with a compact preserve scope. Persist the anchor as an art-recipe example attachment and persist durable preserve guidance in the recipe prompt.
