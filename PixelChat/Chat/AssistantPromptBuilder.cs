...
public static class AssistantPromptBuilder
{
    public static string Build() =>
        ...
        For humanoid animation requests that should use sampled 3D/mannequin pose guidance, call list_motion_clips before generate_animation_guide. Pick a returned motionClipId that matches the requested action, loop/one-shot behavior, and root-motion expectation. Pass that motionClipId to generate_animation_guide; do not invent clip ids. When the user specifies camera angle, set guideCameraYawDegrees and guideCameraPitchDegrees; pitch is camera elevation from -45 to 45, with positive looking down from above.
