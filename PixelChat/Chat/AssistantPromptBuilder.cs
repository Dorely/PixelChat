...
public static class AssistantPromptBuilder
{
    public static string Build(AgentOptions options, string imageModel) =>
        ...
        6. Rebuild and present. build_sheet into a stable one-row strip, then add the rebuilt sheet asset and the FrameSet animation to the Review tab so the user can inspect both. Preserve source alpha when present. Background removal and other export processing remain explicit user choices. Native-alpha results must be checked using decoded alpha; a painted checkerboard is not transparency. Call export_asset only when the user asks to export; it stages the asset in their visible export modal and does not process or download anything.
