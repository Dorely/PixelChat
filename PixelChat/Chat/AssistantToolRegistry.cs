    ...
    IOptions<PixelChat.Art.ImageGenerationOptions> imageOptions)
{
    ...
    public IList<AITool> Build(Guid projectId, AssistantTurnGenerationBudget budget) =>
        ...
        AIFunctionFactory.Create(
            method: (Guid? recipeId,
                ...
                [Description("Generation-only background preference: current, auto, removable, opaque, or transparent (native alpha; follow the selected-model system guidance). Use auto for concept/reference art and removable for export-ready sprite generation.")] string? backgroundPreference = null,
    ...
    private static string SerializeCanvasPreview(EditCanvasPreviewView preview, string targetKind) =>
        JsonSerializer.Serialize(new
        {
            ...
            instruction = "Inspect both images. If placement and the effective editable region are correct, perform exactly one semantic edit using canvasPreparationId and no inline mask/canvas arguments. The preparation locks the submitted inputs, not the provider's returned pixels; inspect the complete edit result afterward.",
        }, JsonOptions);
