...
- OpenAI account OAuth with GPT-6 Astra, Sol, and Luna plus GPT-5.6 Sol, Terra, and Luna chat selections and low through max effort; new account connections default to GPT-5.6 Sol / medium. Built-in models use a 272,000-token context and a 258,400-token input budget, including tools and images, with automatic safe compaction.
...
- Native-alpha requests for the two 2.5 image models, source-preserving edit backgrounds, and image inspection with switchable preview backgrounds, decoded alpha statistics, and pixel RGBA values. Images fit the modal below compact measurements without scrolling. Original provider output can be viewed in the same fitted area.
- Alpha-aware agent vision: every image sent to the assistant includes measured source transparency and an opaque inspection composite. Asset/frame/sprite view tools accept a contrasting background color; chat tool previews retain the exact image the agent saw. Source artwork and provider inputs remain unchanged.
- Configurable OpenAI-compatible chat providers and local SQLite persistence.

Native transparency on both 2.5 models uses `background: "auto"`, PNG output, and explicit alpha instructions injected into the image system prompt. PixelChat keeps the requested `transparent` setting separate from the provider setting and checks returned pixels. September 14, 2026 account tests of the wired system-prompt route produced genuine alpha for generation on both models and a Flare edit; the Sunburst edit returned fully opaque pixels. The explicit `background: "transparent"` parameter was rejected. Opaque results remain available with measured alpha, without automatic cleanup or retries. The assistant receives only the selected image model's guidance in its system prompt.
