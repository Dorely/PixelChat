...
Native transparency sends the documented Responses image tool settings (`background: "transparent"`, `output_format: "png"`). On September 14, 2026, live account-endpoint generation and edit checks of both 2.5 models returned HTTP 400, “Transparent background is not supported for this model.” PixelChat surfaces this access/capability failure without a fallback. Separate diagnostics using `background: "auto"` and explicit alpha prompts returned genuine decoded alpha for generation on both models, but edits of those alpha images returned fully opaque pixels. The app retains the explicit parameter above; it does not silently substitute the diagnostic route. Imported alpha and deterministic processing can still be inspected.
